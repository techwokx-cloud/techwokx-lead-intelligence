    ...
    with st.spinner(f"🔍 Auditing {domain}..."):
        ...
        with tab1:
            ...
            if domain:
                ...
                if ssl_info['valid']:
                    ...
                    with col2:
                        st.metric("Issuer", ssl_info['issuer'][:40])
        ...
        with tab2:
            ...
            if headers_info.get('status_code') == 200:
                ...
                with col2:
                    st.metric("Response Time", f"{headers_info.get('response_time', 0):.0f}ms")
        ...
        with tab3:
            ...
            if 'error' in dns_records:
                st.warning(f"DNS check skipped: {dns_records['error']}")
                st.info("Install dnspython: `pip install dnspython`")
            else:
        ...
        with tab4:
            ...
            try:
                ...
                with col3:
                    st.metric("Final URL", response.url[:50] + "..." if len(response.url) > 50 else response.url)
                    st.metric("Redirects", len(response.history))
                
                # Recommendations
                st.markdown("### Recommendations")
                recommendations = []
                
                if 'ssl_info' in locals() and not ssl_info.get('valid', False):
                    recommendations.append("🔒 Install a valid SSL certificate")
                
                if response_time > 1000:
                    recommendations.append("⚡ Optimize website loading speed")
                
                if 'headers_info' in locals():
                    for header, value in headers_info.get('headers', {}).items():
                        if value == 'Not Set':
        ...
        # SSL check (30 points)
        if 'ssl_info' in locals() and ssl_info.get('valid', False):
            score += 30
...
if not run_audit:
    ...
    with col2:
        if st.button("github.com"):
            st.session_state.example_url = "https://github.com"
            st.rerun()
    
    with col3:
        if st.button("cloudflare.com"):
            st.session_state.example_url = "https://cloudflare.com"
            st.rerun()
    
    # Auto-fill example if selected
    if st.session_state.get('example_url') and not website_url:
        website_url = st.session_state.example_url
        st.rerun()

# Footer
st.markdown("---")
st.caption(f"Website Audit Tool • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
